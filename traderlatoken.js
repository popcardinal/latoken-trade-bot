const RESFINEX = require('./api/latoken_api');
const utils = require('../helpers/utils');

// API endpoints:
// https://api.latoken.com/
const apiServer = 'https://api.latoken.com';
const exchangeName = 'latoken';

module.exports = (apiKey, secretKey, pwd, log, publicOnly = false) => {

  LATOKEN.setConfig(apiServer, apiKey, secretKey, pwd, log, publicOnly);
  ...
  function getMarkets(pair) {
    ...
    return new Promise((resolve, reject) => {
      LATOKEN.markets().then(function(data) {
        try {
  ...
  return {
      ...
      return new Promise((resolve, reject) => {
        RESFINEX.getUserNowEntrustSheet(pair_.coin1, pair_.coin2).then(function(data) {
          try {
            ...
            // this doesn't work as latoken don't update orders' statuses
    ...
    cancelOrder(orderId) {
      /*
        Watch this: some orders on latoken are impossible to cancel, even on latoken UI
    ...
    getRates(pair) {
      pair_ = formatPairName(pair);
      return new Promise((resolve, reject) => {
        LATOKEN.ticker(pair_.pair).then(function(data) {
          data = data.data;
      ...
      if (limit) { // Limit order
        ...
        return new Promise((resolve, reject) => {
          RESFINEX.addEntrustSheet(pair_.pair, coin1Amount, price, side, 'LIMIT').then(function(data) {
            ...
            log.log(`API request LATOKEN.addEntrustSheet-limit(pair: ${pair_.pair}, coin1Amount: ${coin1Amount}, price: ${price}, side: ${side}, 'LIMIT') of ${utils.getModuleName(module.id)} module failed. ${err}.`);
            resolve(undefined);
          });
        });
        ...
        return new Promise((resolve, reject) => {
          RESFINEX.addEntrustSheet(pair_.pair, coin1Amount, '', side, 'MARKET').then(function(data) {
            ...
            log.log(`API request LATOKEN.addEntrustSheet-market(pair: ${pair_.pair}, coin1Amount: ${coin1Amount}, '', side: ${side}, 'MARKET') of ${utils.getModuleName(module.id)} module failed. ${err}.`);
            resolve(undefined);
          });
        });
      }
